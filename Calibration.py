...
class Calibration(object):
    ...
    def brute_force(self, img, color, start_v=25, start_s=13, s_buffer=20, v_buffer=20):
        ...
        for s in xrange(start_s, s_values + 1):
            for v in xrange(start_v, v_values + 1):
                ...
                self.vision.contours = self.vision.get_contours_for_range(img, current_range, vision=self.vision)
                self.vision.apply_all_filters(apply_all=True)
                ratio = self.vision.apply_all_filters(apply_all=False)
                ratings = [1] * len(self.vision.contours)
                for filter_ratio in ratio:
                ...
                if len(ratings) >= self.vision.target_amount:
                    fail = False
                    for cr in ratings[0: self.vision.target_amount]:
                        if type(cr) is ContourRating:
                            if cr.area <= self.min_area:
                                fail = True
                            elif cr.rating < self.rating_ratio:
                                fail = True
                    if fail:
    ...
    def find_data(self, img_list, color=None, start_v=25, start_s=13, s_buffer=20, v_buffer=20):
        ...
        if type(color) is Color:
            sd, sc, vd, vc = [], [], [], []
            for img in img_list:
                tsd, tsc, tvd, tvc = get_data_from_color(color, img, cv2.mean(img))
                sd.extend(tsd)
                sc.extend(tsc)
                vc.extend(tvc)
                vd.extend(tvd)
            if not(len(sd) == len(sc) == len(vc) == len(vd)):
                raise ValueError("Data isn't equal in size!")
            if sd is []:
                raise ValueError("No data")
            self.s_clf = LinearDiscriminantAnalysis(solver='eigen')
            self.s_clf.fit(array(sd), array(sc))
            self.v_clf = LinearDiscriminantAnalysis(solver='eigen')
            self.v_clf.fit(array(vd), array(vc))
            ...
            for img in img_list:
                ...
                for c in color.colors:
                    tsd, tsc, tvd, tvc = get_data_from_color(c, img, mean)
                    sd.extend(tsd)
                    sc.extend(tsc)
                    vc.extend(tvc)
                    vd.extend(tvd)
